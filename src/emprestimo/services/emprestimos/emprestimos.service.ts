...
export class EmprestimosService {
    ...
    async alugarLivro(usuarioId: number, livroId: number): Promise<Emprestimo> {
        const livro = await this.livroRepository.findOne({ where: { id: livroId }});
        ...
        const emprestimoExistente = await this.emprestimoRepository.findOne({
            where: { livro: { id: livroId }, usuario: { id: usuarioId }, dataDevolucao: null }
        });
    ...
    async devolverLivro(usuarioId: number, livroId: number): Promise<Emprestimo> {
        ...
        if (!emprestimo) {
            throw new BadRequestException("O usuário não alugou este livrou ou já devolveu");            
        }
